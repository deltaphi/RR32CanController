...
class DebouncedKey {
 ...
 private:
  ...
  //CyclesType lowCycles; ///< cycles a key needs to remain stable low to be considered debounced-low
  //CyclesType highCycles;  ///< cycles a key needs to remain stable how to be considered debounced-low
  KeyStateType lastStateRead; ///< The last physical state before debouncing that was read
  CyclesType cyclesSinceLastTransition; ///< The number of cycles since the last transition
   ...
   * 
   * This lets the debouncing algorithm assume that teh forces value is the new stable value,
   * e.g., when known from environmental conditions.
   * 
   * If the current physical value does not match the forced value, debouncing will restart
   * with the new physical value.
   */
  ...
  KeyStateType getDebouncedValue() const {
    return this->debouncedKeyState;
  }
